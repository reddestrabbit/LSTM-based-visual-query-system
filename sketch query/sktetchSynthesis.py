...
def nextUserStudyData():
    ...
    min_idxs = argrelmin(np.array(selectedData[index]))
    max_idxs = argrelmax(np.array(selectedData[index]))
    ...
    cubicSpline_data= UnivariateSpline(xSamples,np.array(selectedData[index]))
    cubicSpline_data.set_smoothing_factor(0)
    shift=float(cubicSpline_data(sketchX[index][0]))-sketchY[index][0]
    ...
    for i in range(1,len(sketchX[index])):
        canvas1.create_line(sketchX[index][i], sketchY[index][i]+shift, sketchX[index][i-1], sketchY[index][i-1]+shift, fill='purple',tags='sketch',joinstyle=tk.ROUND, capstyle=tk.ROUND,width=5.5)
...
def nextSythesis():
    ...
    cubicSpline_sketch=UnivariateSpline(np.array(sketchX[index-1]),np.array(sketchY[index-1]))
    cubicSpline_sketch.set_smoothing_factor(0)
    
    global sampleDeltaX
    sampleDeltaX=[]
    minX=sketchX[index-1][0]
    maxX=sketchX[index-1][len(sketchX[index-1])-1]
    tmpCount=minX
    while(1):
        readSamples2(1)
        if tmpCount+sample_2[0]<maxX:
            ...
            break
    global newX,newY 
    newX=[sketchX[index-1][0]]
    newY=[sketchY[index-1][0]+shift]
    #oldX=[sketchX[index-1][0]]
    oldY=[sketchY[index-1][0]+shift]
    for i in range(len(sampleDeltaX)):
        ...
        tmpDiffAngle=np.arccos(tmpx1.dot(tmpx2)/(np.sqrt(tmpx1.dot(tmpx1))*np.sqrt(tmpx2.dot(tmpx2))))
